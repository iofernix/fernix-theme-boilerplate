...
class Theme_Name_Update {
	 ...
	 *
	 * @param array $theme_data    Theme data. Values will be empty if not supplied by the theme.
	 * @since    1.0.0
	 */
	 ...
	 *
	 * @param string $action    The api action to invoke on the update server.
	 * @param array  $params    The parameters for the api call.
	 * @return array            The api response
	 */
	private function call_api( $action, $params ) {
		$api_scheme = wp_parse_url( $this->theme_data->get( 'ThemeURI' ), PHP_URL_SCHEME );
		$api_host   = wp_parse_url( $this->theme_data->get( 'ThemeURI' ), PHP_URL_HOST );
	 ...
	 *
	 * @param mixed $response The api response to verify.
	 ...
	 *
	 * @param mixed $transient  The transient used for WordPress theme / theme updates.
